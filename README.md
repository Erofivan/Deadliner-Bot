# Deadliner Bot 🤖

Telegram-бот для управления дедлайнами с умными напоминаниями и системой весов важности.

## ✨ Функционал

- 📝 **Создание дедлайнов с весами важности**
  - 🔴 Срочно (напоминания каждые 30 мин)
  - 🟠 Важно (каждый час)
  - 🟡 Обычно (каждые 2 часа)
  - 🟢 Несрочно (каждые 4 часа)

- 📋 **Управление дедлайнами**
  - Просмотр списка активных дедлайнов
  - Отметка выполненных
  - Удаление ненужных

- ⏰ **Умные напоминания**
  - Автоматические уведомления по расписанию
  - Ежедневная сводка в 9:00
  - Экстренные уведомления в группы

- 👥 **Работа с группами**
  - Показ дедлайнов в группах по команде
  - Срочные уведомления для всех участников

- 🔑 **Система доступа**
  - Секретный код для получения прав редактирования
  - Безопасное разграничение доступа

- 📤 **Экспорт дедлайнов**
  - Форматированный список для пересылки
  - Удобный формат для печати и планирования

## 🚀 Быстрый запуск

### 1. Подготовка

```bash
git clone https://github.com/Erofivan/Deadliner-bot.git
cd Deadliner-bot
python setup.py
```

### 2. Настройка

Отредактируйте файл `.env`:
```env
BOT_TOKEN=ваш_токен_бота
SECRET_CODE=ваш_секретный_код
```

### 3. Запуск

```bash
python bot.py
```

## 🛠 Получение токена бота

1. Напишите [@BotFather](https://t.me/BotFather) в Telegram
2. Отправьте команду `/newbot`
3. Выберите имя и username для бота
4. Скопируйте полученный токен в `.env` файл

## 📱 Команды бота

### Основные команды
- `/start` - Главное меню с кнопками
- `/help` - Справка по командам
- `/add` - Добавить новый дедлайн
- `/list` - Показать список дедлайнов
- `/export` - Экспорт дедлайнов
- `/code` - Ввести секретный код

### Команды для групп
- `/deadlines` - Показать активные дедлайны группы
- Упоминание бота (`@ваш_бот`) - Показать дедлайны

## 🎯 Форматы ввода даты

Бот понимает различные форматы даты:

```
2024-12-31 15:30
31.12.2024 15:30
31/12/2024 15:30
завтра 15:30
послезавтра 10:00
```

## 📊 Веса важности

| Вес | Эмодзи | Интервал напоминаний | Когда использовать |
|-----|--------|---------------------|-------------------|
| urgent | 🔴 | 30 минут | Критические задачи, горящие сроки |
| important | 🟠 | 1 час | Важные проекты, встречи |
| normal | 🟡 | 2 часа | Обычные задачи, планы |
| low | 🟢 | 4 часа | Долгосрочные цели, идеи |

## 🗂 Структура проекта

```
Deadliner-bot/
├── bot.py              # Основной файл бота
├── database.py         # Работа с базой данных
├── scheduler.py        # Система напоминаний
├── config.py          # Настройки конфигурации
├── setup.py           # Скрипт установки
├── requirements.txt   # Зависимости Python
├── .env.example       # Пример файла окружения
└── README.md         # Документация
```

## 🔧 Зависимости

- `python-telegram-bot==20.7` - Telegram Bot API
- `python-dotenv==1.0.0` - Загрузка переменных окружения
- `apscheduler==3.10.4` - Планировщик задач
- `sqlite3` - База данных (встроена в Python)

## 💡 Примеры использования

### Создание дедлайна
1. Отправьте `/add` или нажмите кнопку "📝 Добавить дедлайн"
2. Введите название: `Сдать отчет`
3. Введите описание: `Квартальный отчет по продажам`
4. Введите дату: `завтра 17:00`
5. Выберите вес: `🟠 Важно`

### Работа в группе
1. Добавьте бота в группу
2. Дайте права администратора (опционально)
3. Используйте `/deadlines` для просмотра активных задач
4. Бот будет присылать срочные уведомления автоматически

### Экспорт дедлайнов
```
🗓 СПИСОК ДЕДЛАЙНОВ
=========================

1. Сдать отчет
   📅 25.12.2024 17:00
   📊 ВАЖНО
   📄 Квартальный отчет по продажам

2. Встреча с клиентом
   📅 26.12.2024 14:00
   📊 СРОЧНО

=========================
Сгенерировано ботом @DeadlinerBot
Дата: 24.12.2024 10:30
```

## 🔐 Безопасность

- Все данные хранятся локально в SQLite базе
- Секретный код защищает от несанкционированного доступа
- Логирование всех операций
- Нет передачи данных третьим лицам

## 🐛 Решение проблем

### Бот не запускается
- Проверьте правильность токена в `.env`
- Убедитесь, что установлены все зависимости
- Проверьте права доступа к файлам

### Напоминания не приходят
- Убедитесь, что планировщик запущен
- Проверьте корректность дат дедлайнов
- Убедитесь, что пользователь не заблокировал бота

### Бот не работает в группе
- Добавьте бота как администратора
- Используйте команды `/deadlines` или упоминайте бота

## 📞 Поддержка

Если у вас возникли вопросы или предложения:
- Создайте Issue в GitHub
- Опишите проблему подробно
- Приложите логи при необходимости

## 📄 Лицензия

MIT License - смотрите файл [LICENSE](LICENSE) для подробностей.

## 🤝 Вклад в проект

Мы приветствуем вклад в развитие проекта!

1. Форкните репозиторий
2. Создайте ветку для новой функции
3. Сделайте коммит изменений
4. Создайте Pull Request

## 🗺 Планы развития

- [ ] Веб-интерфейс для управления
- [ ] Интеграция с календарями
- [ ] Поддержка файлов и медиа
- [ ] Статистика выполнения
- [ ] Командная работа
- [ ] API для интеграций
